...
export function ContactForm() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErrors({ form: "发送失败，请直接发邮件联系我们。" });
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (done) {
    return (
      <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-8 text-center">
        ...
        <h2 className="mt-4 text-xl font-bold text-steel-900">消息已发送</h2>
        <p className="mt-2 text-sm text-slate-600">我们将在 1 个工作日内回复您。</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <label className="mb-1.5 block text-sm font-semibold text-steel-700">姓名 *</label>
        ...
        <div>
          <label className="mb-1.5 block text-sm font-semibold text-steel-700">工作邮箱 *</label>
          <input type="email" className={inputCls} value={values.email} onChange={(e) => update("email", e.target.value)} />
          {errors.email ? <p className="mt-1 text-xs text-red-600">{errors.email}</p> : null}
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-semibold text-steel-700">公司名称 *</label>
        ...
        <div>
          <label className="mb-1.5 block text-sm font-semibold text-steel-700">国家/地区</label>
          <input className={inputCls} value={values.country} onChange={(e) => update("country", e.target.value)} />
        </div>
      </div>
      <div>
        <label className="mb-1.5 block text-sm font-semibold text-steel-700">留言内容 *</label>
