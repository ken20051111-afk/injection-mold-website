...
export async function processQuoteRequest(values: QuoteFormValues) {
  ...
  const rfq = await prisma.rfq.create({
    data: {
      ...
      aiSummary: [
        `${values.materials.length ? `材料：${values.materials.join(", ")}。` : ""}`,
        values.annualVolume ? `年需求量：${values.annualVolume}。` : "",
        values.cavityTarget ? `目标腔数：${values.cavityTarget}。` : "",
      ]
        .filter(Boolean)
        .join(" "),
      aiConfidence: 0.8,
