...
export function ChatWidget() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Hi! I'm the MoldCraft AI sales engineer. Ask me about injection mold capabilities, materials, pricing ranges, or start a quote request.",
    },
  ]);
  ...
  async function send(text?: string) {
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        { role: "assistant", content: "Sorry, our assistant is temporarily offline. Please email sales@moldcraftprecision.com." },
      ]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="fixed bottom-24 right-4 z-50 flex h-[520px] w-[calc(100vw-2rem)] max-w-sm flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl">
          <div className="flex items-center justify-between bg-steel-900 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-white">MoldCraft Assistant</p>
          ...
          {!email && (
            <div className="border-t border-slate-200 bg-white px-3 py-2">
              <input
                ...
                placeholder="Your email (for quote follow-up)"
                type="email"
